...
describe('RateTablesComponent', () => {
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      ...
      .overrideComponent(RateTables2Component, {
  ...
  describe('Rate Table 2 1st TestCase For Name', () => {
    test('check Header & all products Name', fakeAsync(() => {
      ...
      fixture.detectChanges();
      const headerName =
        componentHtml.querySelector('.header .gold')?.textContent;
      expect(headerName).toStrictEqual(component.table[0]?.productName);
      const productsname = componentHtml.querySelectorAll(
        '.product_data_column',
      );
      expect(
        productsname[0]?.querySelector('h3')?.textContent?.trim(),
      ).toStrictEqual(component.table[0]?.BID);
      expect(
        productsname[1]?.querySelector('h3')?.textContent?.trim(),
      ).toStrictEqual(component.table[0]?.ASK);
      expect(
        productsname[2]?.querySelector('h3')?.textContent?.trim(),
      ).toStrictEqual(component.table[0]?.HIGH);
      expect(
        productsname[3]?.querySelector('h3')?.textContent?.trim(),
      ).toStrictEqual(component.table[0]?.LOW);
    }));
    ...
    describe('Rate Table 2 2nd TestCase For classes', () => {
      ...
      it('Rate Default No class', () => {
        const tableElements = componentHtml.querySelectorAll(
          '.product_data_column',
        );
        for (let i = 0; i < 4; i++) {
